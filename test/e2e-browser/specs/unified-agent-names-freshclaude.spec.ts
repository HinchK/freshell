...
// UNIFIED AGENT NAMES (plan Task 8) — the [freshclaude] slice of the six-mode
...
test.describe('[freshclaude] unified agent names', () => {
  test('activity generates one shared short name', async ({ browser }) => {
    await activityGeneratesOneSharedShortName('freshclaude', browser)
  })
  ...
  test('every applicable rename entry converges', async ({ browser }) => {
    ...
    await everyApplicableRenameEntryConverges('freshclaude', browser)
  })
  ...
  test('pending name survives materialize and reopen', async ({ browser }) => {
    ...
    await pendingNameSurvivesMaterializeAndReopen('freshclaude', browser)
  })
  ...
  test('two browsers and an unloaded history page converge', async ({ browser }) => {
    ...
    test.setTimeout(480_000)
    await twoBrowsersAndAnloadedHistoryConverge('freshclaude', browser)
  })

  test('manual name survives late generation and conversation switch', async ({ browser }) => {
    await manualNameSurvivesLateGenerationAndSwitch('freshclaude', browser)
  })
})
