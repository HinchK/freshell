...
// UNIFIED AGENT NAMES (plan Task 8) — the [opencode] slice of the six-mode
...
test.describe('[opencode] unified agent names', () => {
  test('activity generates one shared short name', async ({ browser }) => {
    await activityGeneratesOneSharedShortName('opencode', browser)
  })
  ...
  test('every applicable rename entry converges', async ({ browser }) => {
    ...
    await everyApplicableRenameEntryConverges('opencode', browser)
  })
  ...
  test('pending name survives materialize and reopen', async ({ browser }) => {
    ...
    await pendingNameSurvivesMaterializeAndReopen('opencode', browser)
  })
  ...
  test('two browsers and an unloaded history page converge', async ({ browser }) => {
    ...
    test.setTimeout(480_000)
    await twoBrowsersAndAnloadedHistoryConverge('opencode', browser)
  })

  test('manual name survives late generation and conversation switch', async ({ browser }) => {
    await manualNameSurvivesLateGenerationAndSwitch('opencode', browser)
  })
})
