...
// UNIFIED AGENT NAMES (plan Task 8) — the [codex] slice of the six-mode
...
test.describe('[codex] unified agent names', () => {
  test('activity generates one shared short name', async ({ browser }) => {
    await activityGeneratesOneSharedShortName('codex', browser)
  })
  ...
  test('every applicable rename entry converges', async ({ browser }) => {
    ...
    await everyApplicableRenameEntryConverges('codex', browser)
  })
  ...
  test('pending name survives materialize and reopen', async ({ browser }) => {
    ...
    await pendingNameSurvivesMaterializeAndReopen('codex', browser)
  })
  ...
  test('two browsers and an unloaded history page converge', async ({ browser }) => {
    ...
    test.setTimeout(480_000)
    await twoBrowsersAndAnloadedHistoryConverge('codex', browser)
  })

  test('manual name survives late generation and conversation switch', async ({ browser }) => {
    await manualNameSurvivesLateGenerationAndSwitch('codex', browser)
  })
})
