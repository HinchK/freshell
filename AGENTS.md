...
**Fresh-Agent Orchestration:** The REST agent API (`/api/tabs`, `/api/panes/:id/split`, `/api/panes/:id/send-keys`, `/api/panes/:id/capture`, `/api/panes/:id/wait-for`) and the MCP `freshell` tool accept `agent`/`model`/`effort` parameters to create and drive fresh-agent panes (e.g. `agent=opencode`). The orchestration layer dispatches to the registered `FreshAgentRuntimeManager`, so the same external surface works for any fresh-agent provider. On MCP `new-tab`, resume sugar (`resume`/`resumeSessionId`) is honored for `agent: "opencode"` (Rust server). Agent-resume via `sessionRef` is NOT supported for claude/codex/kilroy agents (the Rust server rejects it with a 400; the Node server silently ignores it) — use an explicit `sessionRef` on MODE panes where that path supports it. Agent-driven tab/pane creation is focus-neutral by contract: server-broadcast ui.command create/split folds carry activate:false into addTab/splitPane and never change the user's active tab/pane — focus moves only via the explicit select-tab/select-pane verbs. The server layout stores mirror the same rule (server/agent-api/layout-store.ts, crates/freshell-freshagent/src/layout_store.rs): agent creates/splits never move the server cursor either, so REST/MCP cursor-relative operations (next/prev tab, omitted targets) keep addressing the user's selection through the mirror window. Client-side this contract has three layers: (1) pane components gate mount-time autofocus on `focusEligible` (tab active + pane active + visible), and non-eligible browser/extension iframes render `inert`; (2) eligible MOUNTS are additionally ownership-gated (`usePaneFocusAdoption` + `src/lib/pane-focus-ownership.ts`): components record at teardown whether their pane held document focus, so an agent-driven leaf→split remount refocuses only if the pane owned focus before — flips (explicit select) always bypass, and same-target re-selects (no eligibility transition) are signaled via a per-pane focus epoch (`panes.focusEpochByPaneId`, nudged only by the select folds, never by pointer mousedowns, and forwarded by PaneContainer as the `focusEpoch` prop); (3) the app-wide focus-steal guard (`src/lib/focus-steal-guard.ts` via `useFocusStealGuard` in App) rebuffs nested-document focus hoists from `data-focus-locked` non-eligible iframes, which `inert` alone cannot stop (Chromium-verified). A store-level selection serial (`paneSelectionMiddleware`) bumps on every `setActivePane`/`nudgePaneFocus`/`setActiveTab` dispatch EXCEPT screenshot capture-internal moves (`capture: true` / `selectTabForCapture`), and — by before/after state diff — on any `addTab`/`switchToNextTab`/`switchToPrevTab`/`splitPane`/`addPane`/`removeTab`/`closePane` that actually moved the selection (agent folds pass `activate:false` and stay serial-invisible; background closes move nothing): restores/descriptors captured before any later selection are voided (newer selection wins, and the screenshot capture aborts a pending focus move instead of stomping a mid-capture selection; screenshot captures are also serialized client-side via a queue: the servers stamp a RELATIVE round-trip budget (ttlMs on the capture frame — no shared wall clock with browser devices) that the client converts to a local deadline at receipt, and queued AND mid-flight captures expire against that deadline instead of mutating UI after the caller already failed; a wedged capture is abandoned at deadline+grace — abandonment runs the job's exactly-once wind-down (restore selection FIRST via the serial-invisible capture actions, then release the refcounted renderer suspension) before the successor starts, and suspension acquisition increments the refcount at ENTRY so a suspension joining mid-paint never re-suspends handlers; restores after a newer user selection are PER-COORDINATE — only coordinates the user actually touched yield (restoreFocus consults the middleware's touched-coordinate map), while capture-owned background coordinates still roll back; interacting with a pane of the currently-visible tab co-touches the tab coordinate, and the capture conversion must NOT treat absolute server epochs as deadline truth (budgets convert only at receipt and are capped at the internal ceiling); user engagement reaching INTO a pane's nested iframe during a capture is observed via focusout/window-blur hoist detection (activeElement becomes the iframe — focusin never fires parent-side for nested engagement, Chromium-verified) COUNTED ONLY when real BOUND input preceded the hoist (a focus-navigation Tab keydown, or pointer activity over the SAME pane — typing/mousing in another pane never launders programmatic eligibility autofocus; autofocus like ExtensionPane's iframe.focus()-on-flip has no input trail and never counts), so a restore never hides the pane the user just clicked and never misfires on capture-induced autofocus; and when a server gives up on a request it broadcasts a screenshot.cancel frame — timeout, gone waiter, OR the first client answering the Rust broadcast (the loser clients' copies are now unanswerable; Node targets a single socket so it only cancels on timeout) — which the client folds into a requestId-keyed cancellation set consulted at queue-dequeue and every staleness gate (delivery order being no guarantee on a stalled client; a cancel landing BEFORE its frame simply consumes the marker without any job ever being enqueued), so capture work never mutates the UI for a caller nobody can answer — mid-render included: every html2canvas render is raced against the request's cancel signal (abandoning a render is side-effect-free, onclone only mutates the clone); wind-down (restore + renderer resume) is a shared promise the abandon timer joins rather than walking past, so a successor never starts mid-restore), records for panes re-appearing in layouts (e.g. reopened tabs) are forgotten, and an eligible mount with focus stranded on `document.body` always uses unknown-pane focus semantics (stranding typing is worse than a rare honest refocus — round-14/16 trade). Locked iframes wake on pointerdown anywhere in the pane shell AND on Enter/Space targeted at the shell itself (`useIframeFocusLock`) — keyboard-parity click-to-wake.
