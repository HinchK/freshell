...
- **Lock authority:** `pnpm-lock.yaml` plus the three demo locks are authoritative. The legacy `package-lock.json` files still exist in-tree during the transition (removed by the migration's final step); they are not install authority — never install from them.
