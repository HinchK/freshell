...
mod tests {
    ...
    /// HARNESS-14 routing proof: with the shared test clock ENABLED, the
    /// 7-day device-display TTL follows virtual `advance_ms()` steps only —
    /// a device pushed BEFORE a virtual 8-day step expires; one pushed AFTER
    /// (same real instant) survives. No backdating into private state.
    #[test]
    fn diagnostic_counts_devicecount_follows_the_shared_test_clock() {
        let _gate = super::test_clock_gate::TestClockGate::enable();
        let reg = TabsRegistry::new();
        ...
        assert_eq!(
            device_count, 1,
            "after a virtual 8-day step, only the post-step device survives the {DEVICE_DISPLAY_TTL_DAYS}-day TTL"
        );
    }
}
