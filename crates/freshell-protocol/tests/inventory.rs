...
fn client_types_match_inventory_exactly() {
    ...
    assert_eq!(
        ...
        Some(42),
        "inventory declares 42 client→server types"
    );
    ...
    assert_eq!(actual.len(), 42, "crate declares 42 client types (no dups)");
    assert_eq!(
        actual, expected,
...
fn server_types_match_inventory_exactly() {
    ...
    assert_eq!(
        ...
        Some(65),
        "inventory declares 65 server→client types"
    );
    ...
    assert_eq!(actual.len(), 65, "crate declares 65 server types (no dups)");
    assert_eq!(
        actual, expected,
...
#[test]
fn combined_surface_is_107() {
    let all = all_message_types();
    assert_eq!(all.len(), 107, "42 client + 65 server = 107 discriminants");
    // sorted + unique
