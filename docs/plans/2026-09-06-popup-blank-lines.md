...
- **Half 1 — provider carve-out** (`src/components/context-menu/ContextMenuProvider.tsx`):
  - `handleContextMenu` (capture phase) returns early — no `openMenu` — when the event belongs to a turn gesture. It DOES call `preventDefault()` (when cancelable) on that early return: when the event targets the turn article, the transcript's own handler cancels it anyway (harmless double cancel); but a LATE retargeted event that lands on the sheet has no transcript handler at all, and without `preventDefault` the browser would show its NATIVE context menu over the sheet (delta-review round 3).
