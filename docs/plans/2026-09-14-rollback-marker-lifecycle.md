...
- Frozen contract workflow for any snapshot schema change: edit `shared/ws-protocol.ts`, run `npm run contract:generate`, update `crates/freshell-protocol` inventory arrays/counts, and commit regenerated `port/contract/*.json` in the same commit.
...
- `codex.rs` `handle_rollback_undo_send_undo_undo_freezes_epoch_zero_and_orders_the_new_epoch_ascending` (~12764): at the test's tail (after the existing entries asserts ~12853-12868), build the snapshot and assert: EVERY bucket turn `restorable == false` and `rollback.canRedo == false` (codex can never be restorable).
