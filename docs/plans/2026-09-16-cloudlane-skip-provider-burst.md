...
| freshopencode-db-history (5prk, 3 failures) | keep skip-listed | Task 3: scoped strict-mode-safe transcript locators (the :245 strict-mode violation) + the audit pair re-specified to the serve contract (`prompt_async`, `run` pinned absent); the two polls/waits keep their original budgets — the planned 30s→60s raises were dropped by the amendment (the failures were a deterministic stale-contract layer, not latency) |
