...
### Requested result
Execute the committed Freshell pnpm migration plan at `docs/plans/2026-09-19-pnpm-migration.md` to completion via the-usual in a dedicated worktree: replace npm for Freshell's first-party dependency installation and development/build/test workflows with exactly pinned pnpm 10, one workspace lock plus three independent demo locks, pnpm-deploy-based portable Electron/sidecar/MCP runtime packaging, and migrated CI/cloud/sandbox/docs surfaces — without changing application behavior — ending with a clean committed feature branch ready for PR (PR creation requires explicit user approval).

### Explicit constraints
- Run the full the-usual workflow: dedicated worktree branch from origin/main, external run record, load-bearing validation, independent plan Fresh Eyes (max 3 rounds), task-by-task execution with fresh implementers and task reviews, whole-branch review, final delta Fresh Eyes (max 5 rounds), recap.
- Base per user waiver (2026-09-20): build from the then-current origin/main without waiting for a green base gate (waived after two red full-suite gates whose output belongs to a concurrent run); the red state is an accepted pre-existing condition, and the run must introduce no further regressions — a failure counts as pre-existing only with a reproduction receipt at base_ref, and no new failure may be attributed to this migration.
- Pin pnpm 10.34.5 exactly everywhere (root, demos, CI, Docker, sandbox, native Windows); frozen installs (`pnpm install --frozen-lockfile`); mismatched/missing locks must fail, no silent mutable recovery; no broad dependency upgrades, audit fixes, or dedup during conversion.
- Preserve application behavior and boundaries: Rust backend, source directories, public commands, separate sidecar/MCP processes, runtime resource paths, supported platforms, test coordination; no new Node service; do not combine sidecar with the app runtime.
- Do not rename non-npm things: keep Electron-builder's `npmRebuild: false`, `npm_*` compatibility env variables, npm registry URLs, third-party/global-CLI npm usage, and intentional historical npm references.
- Red/green/refactor TDD for behavior changes; remove text-only npm-string assertions (plan 9.3) rather than rewriting them as pnpm-string assertions; never reduce coverage or skip tests.
- Broad repo-supported test runs wait for the shared coordinator gate; cloud Vitest/e2e backends per the user's standing choice; never silently fall back to local after a cloud failure.
- Do not restart or deploy the live self-hosted server (port 3001) without explicit "APPROVED"; do not migrate the live main checkout's dependency tree during this run; live-checkout transition is a separate explicitly authorized maintenance operation.
- Never push behavior changes directly to main; push the feature branch only, and create/merge the PR only after explicit user approval.
- Preserve the pre-push Git environment sanitization and fixture/Cargo routing fixes from PR #809.

### Accepted tradeoffs and residuals
- Native Windows and macOS packaging/build proofs require their actual supported runners; WSL evidence does not substitute (plan sections 6.3, 9.1). Availability of those runners bounds when those proofs can be recorded.
- Historical npm references intentionally remain in generic terminal/transcript tests, provider shims, registry URLs, npm_* compatibility variables, old command receipts, and classified historical docs; a repo-wide zero-occurrence sweep is explicitly not an acceptance criterion.
...
Date: 2026-09-19. Status: implementation plan; load-bearing validation completed 2026-09-20 (see the validation notes in sections 3.1-3.3 and 6.2, and the run's load-bearing ledger); the migration itself has not been performed.
...
While that gate ran, origin/main advanced to 9bdc8f63e (PR #809). Its five-file delta was inspected separately. It fixes Git environment leakage from the pre-push hook and expands fixture/Cargo routing. Preserve those fixes during implementation; this plan's branch remains on the exact fully tested base. Start implementation from the then-current origin/main after its required base gate. (2026-09-20 update: subsequent full-suite base gates at 9bdc8f63e and 5bd698342 failed (test-side flakes; concurrent agents are landing fixes, e.g. PRs #811, #813, #814), and the user waived the green-base precondition — build from the then-current origin/main with the no-further-regressions obligation recorded in the User Request block. The implementation branch is based on 8255c0fb9; PRs #809-#814 are all preserved in it.)
...
Corepack-shim caveat (verified 2026-09-20): on hosts where Corepack is enabled, the `pnpm` command may be a Corepack shim that resolves the pnpm version FROM the manifest's packageManager field and downloads it on demand — this bypasses pnpm's own packageManagerStrictVersion/managePackageManagerVersions posture at the shim layer, so a "wrong version present" cannot fail through the shim. The migration's bootstrap installs the real pinned binary via npm i -g (which replaces the shim), and the wrong-version failure verification (section 3.2) must run the real installed binary, not a shim. Bootstrap documentation and CI guidance must call out Corepack-enabled hosts, and the version check must not silently succeed via a shim download. [Pnpm 10 installation](https://pnpm.io/10.x/installation), [Corepack project documentation](https://github.com/nodejs/corepack).
...
Pnpm 10.34.5 supports allowBuilds for dependency install scripts; unlisted scripts are blocked/warned, not automatically approved, and blocking is warn-not-fail (install completes; the warning lists the skipped packages and suggests pnpm approve-builds as remediation). allowBuilds keys by package NAME and one name key covers all installed versions of that package (both esbuild instances run under one esbuild: true entry); it does not match file: protocol tarball dependencies by name — irrelevant here because every real allowBuilds entry (electron, esbuild, fsevents) is a registry package. enablePrePostScripts separately controls project script hooks, which startup preparation and the production build guard need, and its observed default is ON in 10.34.5 even though the run handler falls back to a false default — set it explicitly everywhere rather than relying on defaults. Consider strictDepBuilds after classifying known dependencies. Do not mix competing old onlyBuiltDependencies settings into this policy. [Pnpm 10 settings](https://pnpm.io/10.x/settings).
...
4. Compare versions, peer contexts, integrity data, and platform-specific optional dependencies against both original graphs and the independent conversions. In particular, preserve the SDK/Anthropic/MCP/Zod versions listed in section 2.2 and all required SDK platform packages. Use supported pnpm resolution with explicit declarations/targeted constraints to obtain the reviewed combined graph where importing alone cannot. Do not manually invent pnpm snapshot keys. Document unavoidable differences individually. This diff step is REQUIRED, not precautionary (verified 2026-09-20): a workspace-root pnpm import does not merge member npm locks, so the sidecar importer resolves fresh and its transitive closure drifts from the npm-era snapshot (observed in a disposable conversion probe: standardwebhooks 1.0.0→1.1.1, plus minor-version differences in hono/jose/express-rate-limit/body-parser/qs/type-is/ajv-8/@babel/runtime lines and npm-only cjs-alias entries). Every drifted package must be individually reviewed: pin it with a targeted constraint where the plan requires the exact locked version, or record the accepted difference. The root importer's peer-pinned versions (SDK 0.3.237, @anthropic-ai/sdk 0.120.0, MCP 1.30.0, zod 4.4.3 vs root's 4.3.6) are preserved exactly by import plus explicit peer declarations — verified with peer-suffixed snapshot contexts in the converted lock.
...
In the inspected pnpm 10 implementation, normal shared-lock deploy produces a filtered lock and installs it frozen. It requires a workspace with injectWorkspacePackages enabled and localizes the virtual store. The legacy path can run with frozenLockfile false, so do not silently use --legacy to get past a configuration failure. Verified 2026-09-20 on pnpm 10.34.5 with this workspace's real manifests/locks in a disposable probe: both deploys succeed without any prior full workspace install (they resolve from the workspace lock); the deployed trees contained every dependency/peer/optional instance (including the Claude SDK's linux-x64 native optional package), had zero external or broken links, and executed their entrypoints after the source fixture and its store were physically moved away. The deploy lock's importer graph is correctly filtered to the deployed package, while its packages map retains whole-workspace entries and only the importer's closure materializes on disk — verification must validate the importer closure and the staged receipt, not a "every locked package has a node_modules/.pnpm directory" sweep. [Pnpm deploy](https://pnpm.io/10.x/cli/deploy), [pinned deploy implementation](https://github.com/pnpm/pnpm/blob/v10.34.5/releasing/plugin-commands-deploy/src/deploy.ts).
