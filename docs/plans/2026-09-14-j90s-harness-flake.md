...
## Amendment (2026-09-14, execution cycle 3): wedge-tolerant self-healing boot wait

Execution evidence superseded two sizing assumptions in this plan. Two acceptance runs of the kata invocation (17:29Z workers:2 at 801c3a807; 18:11Z workers:1 at e243a4b06) both still flaked the settings test on attempt 1 — same 46000ms `waitForConnection` ceiling, once at the post-reload leg, once at the fixture-level initial wait with zero sibling contention — and each warm retry passed in ~6s. A read-only investigation (reports/stall-investigation.md) established, with Cloud Monitoring and log evidence:

- **Root cause**: rare ~40-60s zero-CPU wedges of in-flight server I/O in the e2e Node TestServer under gVisor (Cloud Run's sandbox). CPU was 7.6% of quota during a stall core (not busy, not throttled); a sibling fetch to a fully in-memory endpoint hung ≥58s without erroring.
- **Why the wait hangs**: `waitForConnection` gates on the client boot chain — 4-5 sequential, timeout-less HTTP fetches (App.tsx:1690-1753) run before `ws.connect()`. One wedged fetch ⇒ the WS never starts ⇒ the predicate can never be satisfied, regardless of window size.
- **Wedges self-heal**: all observed episodes ended within ~6-10s of the failed wait, and fresh boots complete in ~6s afterwards.
- **Server-side telemetry was invisible**: the TestServer's stdout/stderr go to in-memory buffers (test-server.ts:473-483 re-emit only when `verbose`, never set) — which is why diagnosing cost two extra cloud cycles.

The already-landed pieces (real timeout binding, env-scalable window, cloud lane env plumbing, sequential lane) are confirmed working end-to-end in the cloud lane (the observed 46000ms timeout is exactly the env value + slack) and stay. What changes:

1. **The cloud window grows to 90000 ms** and the settings:185 test gets a cloud-only 120s per-test budget — sized to the observed wedge class (≤~60s) plus a ~6s fresh-boot recovery plus the test body.
2. **`waitForConnection` gains an opt-in, one-shot mid-wait self-heal** (kata direction 1's "bounded retry/extend"): when enabled and ready has not landed by half the window, reload the page once and keep waiting with the remaining budget. A fresh boot chain is the observed recovery path. Opt-in only — enabled at fresh-boot call sites (the `freshellPage` fixture and the settings:212 reload leg), never at mid-test recovery waits where a reload would destroy state under test. The ready assertion itself is unchanged: the test still fails if the app genuinely cannot become ready inside the window. No retry-count hiding, no assertion loosening.
3. **TestServer logs reach the container log stream on the cloud lane**: the fixtures read `FRESHELL_E2E_SERVER_VERBOSE` and pass the existing `verbose` hook (test-server.ts:473-483); the cloud env file sets it. This is the observability the flake class was missing — the next wedge becomes diagnosable from Cloud Logging.
4. **`workers: 1` stays** (landed at e243a4b06): it did not fix the flake (wedge is contention-independent) but it removes a demonstrated variance source (sibling CPU bursts feeding the client's 10s ready-watchdog amplifier), and the lane remains far faster than local.

Residual, accepted and recorded: a wedge longer than ~84s would still flake the settings test on attempt 1 (absorbed by `retries: 2`); no in-repo means can bound an infra-layer wedge of arbitrary length. Every OBSERVED wedge (40-60s) now survives attempt 1.

### Task 4: Wedge-tolerant self-healing boot wait + evidence-sized cloud budgets + server-log visibility

**Files:**
- Modify: `test/e2e-browser/helpers/test-harness.ts` (waitForConnection self-heal), `test/e2e-browser/helpers/test-harness.test.ts` (new cases), `test/e2e-browser/helpers/fixtures.ts` (opt-in + verbose flag), `test/e2e-browser/specs/settings.spec.ts` (opt-in at :212, cloud-only 120s timeout at :185), `scripts/e2e-cloud.sh` (90000 default + verbose flag emission + usage), `scripts/test/e2e-harness-timeout-env.test.sh` (90000 + verbose assertions)

**Interfaces:**
- Consumes: Task 1's resolver (unchanged); the existing `verbose` option on the e2e server handle (test-server.ts:473-483 hook — no test-server.ts change).
- Produces: `WaitForConnectionOptions { selfHealReload?: boolean }`; `waitForConnection(timeoutMs?: number, opts?: WaitForConnectionOptions)`. Cloud env: `FRESHELL_E2E_WS_READY_TIMEOUT_MS` default 90000, `FRESHELL_E2E_SERVER_VERBOSE=1`.

- [ ] **Step 1: Write the failing behavioral tests** — extend `test-harness.test.ts`: fakePage gains a `reload` recorder and per-call waitForFunction outcome control. New cases: (a) self-heal OFF (default): a never-ready predicate produces exactly ONE waitForFunction call and NO reload (all 184 existing call sites keep semantics); (b) self-heal ON + ready within phase 1: NO reload; (c) self-heal ON + phase-1 timeout: exactly ONE reload, then a second waitForFunction whose options timeout = window − floor(window/2); (d) explicit timeout + self-heal: phases derive from the explicit window; (e) self-heal ON + both phases time out: the final TimeoutError propagates (test asserts rejection).
- [ ] **Step 2: Run and verify the intended failure** — `npm run test:e2e:helpers -- test-harness`: FAIL on the missing self-heal behavior (reload recorder never fires / no options parameter exists).
- [ ] **Step 3: Implement** — `waitForConnection` with the self-heal branch (phase 1 = `pollReadyWithin(floor(W/2))` returning boolean; on false: one `await this.page.reload({ timeout: W - floor(W/2) })`; final `awaitReady(W - floor(W/2))` throws natively); fixtures.ts `freshellPage` calls `waitForConnection(undefined, { selfHealReload: true })` and passes `verbose: process.env.FRESHELL_E2E_SERVER_VERBOSE === '1'` to the server handle; settings.spec.ts `:212` call opts in, and the `:185` test declaration gains `...(process.env.FRESHELL_E2E_WS_READY_TIMEOUT_MS ? { timeout: 120_000 } : {})` options (cloud-only; locally undefined → default 60s); e2e-cloud.sh emission becomes `FRESHELL_E2E_WS_READY_TIMEOUT_MS: "${FRESHELL_E2E_WS_READY_TIMEOUT_MS:-90000}"` plus a new `FRESHELL_E2E_SERVER_VERBOSE: "1"` line, comment rewritten to the wedge narrative, usage text updated (90000; documents both vars); bash test asserts 90000 default, 60000 override, and `FRESHELL_E2E_SERVER_VERBOSE: "1"`.
- [ ] **Step 4: GREEN** — `npm run test:e2e:helpers -- test-harness` (all cases incl. the prior 7).
- [ ] **Step 5: Refactor while green** — none expected; state why in the report.
- [ ] **Step 6: Impacted verification** — full `npm run test:e2e:helpers`; `bash scripts/test/e2e-harness-timeout-env.test.sh`; `npm run typecheck && npm run lint` (cross-tree guards).
- [ ] **Step 7: Commit** — `git add` the six files; message: `test(e2e): wedge-tolerant self-healing boot wait + evidence-sized cloud budgets (j90s)`.
- [ ] **Step 8: Acceptance re-run** — the kata invocation on cloud (exit-capture block, `GCLOUD_IDENT` exported): REQUIRED: settings:185 passes attempt 1, no flaky annotation. Then the Step 3 gates from Task 3: coordinator-checked `npm test` (cloud backend; wait for foreign holders), `npm run test:e2e:helpers`, `bash scripts/test/e2e-harness-timeout-env.test.sh`. Record all evidence.
