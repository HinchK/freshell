...
- **Impacted-test sets must include every consumer** (per the blast-radius inventory in `.the-usual-logs/fab-setting-v2/reports/plan-default-flip-blast-radius.md`): the settings-layer suites (`settingsSlice.test.ts`, `state-edge-cases.test.ts`, `browserPreferencesPersistence.test.ts`, `crossTabSync.test.ts`), the bootstrap-path suites (`App.test.tsx`, `test/e2e/terminal-font-settings.test.tsx`), the consumption suites (`PaneLayout.test.tsx`, `SettingsView.panes.test.tsx`, `editor-pane.test.tsx`), the e2e lane specs (`settings.spec.ts`, `mobile-viewport.spec.ts`, `pane-picker.spec.ts`, `screenshot-baselines.spec.ts`), and the ~18 untouched groups are explicitly re-run or argued green in each task's Step 6.
...
Expected: FAIL — exactly four tests fail because the option and the `previousPatch` parameter do not exist yet and are silently ignored by BOTH functions: `defaults to true when the desktop platform default is provided` (resolution ignores the option and returns the static `false`); `persists ... (desktop base)` (both halves fail against the static base: the `off` value diffs `false === false`, produces no entry, and `toBe(false)` fails; the `on` value diffs `true !== false`, produces a spurious entry, and `toBeUndefined()` fails); `preserves an explicitly saved false across a flush under the platform default it equals` (third argument ignored, the resolved value is `false` from the explicit patch, it equals the static `false`, the entry is omitted, and `toBe(false)` fails on `undefined`); and `keeps an explicit value change flowing through the sticky rule` (third argument ignored, the resolved `false` equals the static `false`, the entry is omitted, `toBe(false)` fails). The other six new tests are companion pins that pass before implementation and must keep passing after: the mobile-default pin and the explicit-over-platform pin pass either way (the static default is also `false`); the mobile-base diff test passes either way (`true` already differs from the static `false`, `false` already equals it); the old-blob pin passes because explicit patches already win today; the sticky-true-desktop pin passes because `true` already differs from the static `false`; and `omits a fresh platform-default value when no explicit choice was ever saved` passes pre-implementation because the ignored resolution option materializes the static `false`, which the ignored diff option then omits against the static base — it becomes the no-poisoning guard only under the implemented rule (fresh desktop `true` diffed against the platform `true` stays out of the blob). All ten pre-existing tests in the describe stay green (their zero-arg calls are unchanged).
...
- Test: `test/unit/client/store/crossTabSync.test.ts:816` (fixture update: resolve with the desktop platform default)
- Test: `test/unit/client/components/App.test.tsx:184` (fixture update: same)
- Test: `test/e2e/terminal-font-settings.test.tsx:107` (fixture update: same)
...
- `test/unit/client/store/crossTabSync.test.ts:816` — fixture update: the test dispatches a hand-materialized store value; zero-option resolution pins the static `false`, which the new desktop-threaded write-diff would record as an explicit disable, adding an unexpected FAB key to the exact blob assertion at `:841`. Resolve with the desktop platform default instead (what a real desktop boot materializes — jsdom is desktop-ambient), and the blob stays FAB-free:
...
```
  ...
  (The other `setLocalSettings(resolveLocalSettings(...))` dispatch at `:588` is safe as-is: that test's store is built WITHOUT the persistence middleware, so its dispatch never flushes a blob.)

- `test/unit/client/components/App.test.tsx:184` — fixture update: the settings-state helper materializes local settings with zero-option `resolveLocalSettings(options.local)`; change it to resolve with the desktop platform default so the legacy-seed bootstrap tests (which exercise the modified App.tsx:674 path) keep seeing a store whose FAB value equals the boot default rather than a spurious explicit disable:

```ts
  const localSettings = resolveLocalSettings(options.local, { floatingActionButtonDefault: true })
```

  (App.test.tsx's exact assertions are all localStorage-blob `toEqual`s — `:1086`, `:1210`, `:1329`, `:1369` — and the FAB key stays out of every one of them under the platform-coherent diff; no other assertion touches the FAB.)

- `test/e2e/terminal-font-settings.test.tsx:107` — fixture update: `createSettingsState` has the same zero-option materialization; without the fix, the store's static `false` looks like an existing explicit setting to the new desktop-aware bootstrap diff at App.tsx:674, `shouldPersistCurrentLocalSettings` flips, and the legacy font seed is skipped. Resolve with the desktop platform default:
...
Run: `FRESHELL_VITEST_BACKEND=cloud FRESHELL_E2E_BACKEND=cloud FRESHELL_GCP_ACCOUNT=gcloud-robot@misc-puttering-project.iam.gserviceaccount.com npm run test:vitest -- run test/unit/shared test/unit/client/store test/unit/client/components/panes test/unit/client/components/SettingsView.panes.test.tsx test/unit/client/components/App.test.tsx test/integration/client test/e2e/refresh-context-menu-flow.test.tsx test/e2e/pane-context-menu-stability.test.tsx test/e2e/terminal-url-context-menu.test.tsx test/e2e/terminal-font-settings.test.tsx`

Run: `npm run typecheck:client && npm run lint`

Expected: PASS for both. Green-by-construction arguments for the untouched groups (cite in the task record): `crossTabSync.test.ts` asserts key-scoped settings and exact blobs whose FAB entries stay absent — its one middleware-flushing dispatch is fixture-updated to the desktop platform default (the `:588` dispatch never flushes); `App.test.tsx` exercises the modified legacy-seed bootstrap path with the fixture-updated helper, and its four exact-blob assertions stay FAB-free under the platform-coherent diff; `FloatingActionButton.test.tsx` is props-only; the three context-menu jsdom flows mock `FloatingActionButton` to `null` and preload `defaultSettings` (static composite); App-level jsdom suites other than App.test.tsx mock `TabContent` so no real `PaneLayout` mounts; `editor-pane.test.tsx` dispatches the enable explicitly; suites that preload `defaultSettings` keep the FAB out of unrelated assertions; the shared stream-deck and settings per-key describes use zero-arg calls (static base, unchanged).
...
Run: `FRESHELL_VITEST_BACKEND=cloud FRESHELL_E2E_BACKEND=cloud FRESHELL_GCP_ACCOUNT=gcloud-robot@misc-puttering-project.iam.gserviceaccount.com npm run test:e2e:update-snapshots -- --project=chromium --update-snapshots=all test/e2e-browser/specs/screenshot-baselines.spec.ts`

(The bare-script form shown in the original draft is the structural no-op described above — the `=all` mode is REQUIRED for this task's purpose; as executed, this command is what actually rewrote the stale baselines.)
