...
**Plan addition (certification evidence, 2026-09-02):** certification run 1 surfaced a second, budget-independent load failure in `rate_limited_retry_same_requestid_proceeds` (restore_spawn_gate.rs:750): with a 300ms rate window, server-side processing lag on a loaded host can let the second create's rate check land AFTER the window slid — the expected `RATE_LIMITED` error frame never exists. Load-safe equivalent formulation (no assertion weakened; the sentinel-cleanup property now verified even more directly):
1. Convert the two RATE_LIMITED-proof reads into a TWICE-limited shape: after the first `RATE_LIMITED` error for `rl-2`, immediately resend `rl-2` (no window slide yet) and assert a SECOND `RATE_LIMITED` error arrives. A leaked InFlight sentinel would swallow the resend (waiter registration, no error frame) and the bounded loop fails — so this probes sentinel cleanup with zero dependence on the 300ms boundary.
2. Keep the existing post-window-slide resend (`sleep(400ms)` > `rate_window_ms: 300`, then assert `terminal.created`) — that step is load-safe by construction: the client's 400ms sleep starts only after rl-1's `terminal.created` was RECEIVED, and the server's rate window starts at rl-1's token consumption (≤ that receipt), so any server lag only lands the resend further past window start.
3. The A2 comment block above the test stays (requirement statement unchanged); add one DEFLAKE paragraph explaining the formulation change and citing certify run 1.
